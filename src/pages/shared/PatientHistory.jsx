	  ...
	  <div className="print:hidden relative max-w-7xl mx-auto space-y-8 p-4 md:p-8 font-sans min-h-screen overflow-hidden">
		...
		<div className="relative z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-lg flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 transition-all">
			<div>
				...
				<h1 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Patient Database</h1>
			</div>
		...
		{/* ALERTS */}
		{error && (
		  <div className="relative z-10 p-4 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 font-bold rounded-xl border border-red-200 dark:border-red-900/50 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
			<ShieldAlert className="w-5 h-5"/> {error}
		  </div>
		)}
		{successMsg && (
		  <div className="relative z-10 p-4 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 font-bold rounded-xl border border-emerald-200 dark:border-emerald-900/50 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
		  ...
		  <div className="relative z-10 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xl flex flex-col min-h-[600px] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
			<div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
			  <h3 className="font-bold text-sm uppercase tracking-widest text-slate-600 dark:text-slate-400 flex items-center gap-2">
				<Users className="w-5 h-5 text-blue-500"/> Registered Patients Directory
			  </h3>
			...
			<div className="flex-1 overflow-x-auto">
				  ...
				  {filteredDirectory.map(p => (
					<tr key={p.id} onClick={() => loadPatientTimeline(p.id)} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-all group">
					  <td className="px-6 py-4">
						...
						<div className="text-[10px] font-mono text-slate-400 mt-0.5">ID: {p.id.substring(0, 12)}...</div>
					  </td>
					  <td className="px-6 py-4 text-slate-600 dark:text-slate-300 font-medium">
						<div className="flex items-center gap-2"><Phone className="w-3 h-3 text-slate-400" /> {p.phone || 'N/A'}</div>
					  </td>
					  <td className="px-6 py-4">
						<div className="flex items-center gap-2">
						  <span className="text-slate-700 dark:text-slate-200 font-medium">{p.date_of_birth || 'N/A'}</span>
						  <span className="text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-full uppercase">{p.sex || '?'}</span>
						</div>
					  </td>
			  ...
			  {filteredDirectory.length === 0 && !loading && (
				<div className="text-center py-20 flex flex-col items-center justify-center">
				  ...
				  <div className="text-slate-500 font-bold text-sm">No patient records found.</div>
				  <div className="text-slate-400 text-xs mt-1">Try adjusting your search criteria.</div>
				</div>
			  )}
			</div>
		  </div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
					...
					<div className="lg:col-span-4 space-y-6">
						...
						<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-lg">
						  <div className="bg-blue-600 p-6 text-white text-center">
							...
							<div className="font-mono text-[9px] uppercase tracking-widest text-blue-200 bg-blue-900/30 inline-block px-3 py-1 rounded-full border border-blue-500/30 break-all">ID: {patientData.id}</div>
						  </div>
						  <div className="p-6 space-y-4">
							<div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
								<div className="flex items-center gap-3 text-slate-500"><Calendar className="w-4 h-4" /> <span className="text-xs font-bold uppercase tracking-wide">DOB / Sex</span></div>
								<div className="font-black text-sm text-slate-900 dark:text-white text-right">{patientData.date_of_birth || 'N/A'} <span className="text-slate-400">({patientData.sex || '-'})</span></div>
							</div>
							<div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
								<div className="flex items-center gap-3 text-slate-500"><Droplet className="w-4 h-4 text-red-400" /> <span className="text-xs font-bold uppercase tracking-wide">Blood Type</span></div>
								<div className="font-black text-lg text-red-600">{patientData.blood_group || 'Unknown'}</div>
							</div>
							<div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
								<div className="flex items-center gap-3 text-slate-500"><Phone className="w-4 h-4" /> <span className="text-xs font-bold uppercase tracking-wide">Contact</span></div>
								<div className="font-black text-sm text-slate-900 dark:text-white">{patientData.phone || 'N/A'}</div>
							</div>
						  </div>
						</div>

						{/* ATTACHED RECORDS ZONE */}
						<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-lg">
						  <h3 className="font-black text-slate-900 dark:text-white flex items-center gap-2 mb-5 pb-4 border-b border-slate-100 dark:border-slate-800">
							<Paperclip className="w-5 h-5 text-emerald-500"/> External Records
						  </h3>
						  ...
						  <label className={`block w-full border-2 border-dashed ${uploadingFile ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'border-slate-200 dark:border-slate-700 hover:border-emerald-400 hover:bg-slate-50 dark:hover:bg-slate-800'} rounded-2xl p-6 text-center cursor-pointer mb-5 transition-all group`}>
							  ...
							  {uploadingFile ? (
								...
								<UploadCloud className="w-8 h-8 mx-auto mb-3 text-slate-300 group-hover:text-emerald-500 transition-colors group-hover:-translate-y-1"/>
							  )}
							  <div className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest">{uploadingFile ? 'Encrypting & Uploading...' : 'Click to Upload Document'}</div>
						  </label>
						  
						  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1 custom-scrollbar">
							  {documents.length === 0 ? (
								<div className="text-center py-6 text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-50 dark:bg-slate-950 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">No Files Attached</div>
							  ) : documents.map(doc => (
								  <a key={doc.id} href={doc.file_url} target="_blank" rel="noreferrer" className="flex items-center justify-between p-3.5 bg-slate-50 dark:bg-slate-950 hover:bg-white dark:hover:bg-slate-800 hover:shadow-md transition-all rounded-xl border border-slate-100 dark:border-slate-800 group">
									  <div className="flex items-center gap-3 overflow-hidden">
										  <div className="bg-slate-200 dark:bg-slate-800 p-2 rounded-lg group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/30 transition-colors">
											<FileText className="w-4 h-4 text-slate-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
										  </div>
					...
					<div className="lg:col-span-8">
						<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-lg min-h-full">
							<h3 className="font-black text-xl text-slate-900 dark:text-white flex items-center gap-3 mb-8 pb-4 border-b border-slate-100 dark:border-slate-800">
							  <Activity className="w-6 h-6 text-blue-500"/> Clinical Timeline
							</h3>
							
							{timelineEvents.length === 0 ? (
								<div className="text-center py-20">
								  <Clock className="w-12 h-12 text-slate-200 mx-auto mb-4" />
								  <div className="text-slate-500 font-bold">No clinical history recorded.</div>
								</div>
							) : (
									...
									{timelineEvents.map((event, index) => {
											...
											<div key={`v-${event.id}`} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
												...
												<div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all">
													<div className="flex justify-between items-center mb-3">
														<div className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 dark:bg-blue-900/20 dark:text-blue-400 px-3 py-1 rounded-full">{event.visit_type} Visit</div>
													...
													<div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-3 border border-slate-100 dark:border-slate-800 mb-4 grid grid-cols-3 gap-2 divide-x divide-slate-200 dark:divide-slate-700 text-center">
													  <div><div className="text-[9px] text-slate-400 font-bold uppercase mb-0.5">BP</div><div className="font-mono text-xs font-black text-slate-700 dark:text-slate-300">{event.blood_pressure || '--/--'}</div></div>
													  <div><div className="text-[9px] text-slate-400 font-bold uppercase mb-0.5">HR</div><div className="font-mono text-xs font-black text-slate-700 dark:text-slate-300 flex items-center justify-center gap-1"><HeartPulse className="w-3 h-3 text-rose-400"/>{event.heart_rate || '--'}</div></div>
													  <div><div className="text-[9px] text-slate-400 font-bold uppercase mb-0.5">Temp</div><div className="font-mono text-xs font-black text-slate-700 dark:text-slate-300">{event.temperature || '--'}</div></div>
													</div>

													{event.diagnosis && (
														<div className="bg-blue-50/50 dark:bg-blue-900/10 rounded-xl p-4 border border-blue-100 dark:border-blue-900/30">
															<div className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-1.5">Diagnosis & Rx</div>
															<div className="font-bold text-sm text-slate-800 dark:text-slate-200 mb-2">{event.diagnosis}</div>
															<div className="text-xs font-medium text-slate-600 dark:text-slate-400 italic bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-100 dark:border-slate-800">{event.prescribed_meds || 'No prescriptions issued.'}</div>
														</div>
													)}
												</div>
											</div>
										...
										if (event._type === 'lab') return (
											<div key={`l-${event.id}`} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
												...
												<div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 rounded-2xl border bg-white dark:bg-slate-950 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all ${event.lab_type === 'Radiology' ? 'border-purple-200 dark:border-purple-900/50' : 'border-rose-200 dark:border-rose-900/50'}`}>
													<div className="flex justify-between items-center mb-3">
														<div className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${event.lab_type === 'Radiology' ? 'text-purple-600 bg-purple-50 dark:bg-purple-900/20' : 'text-rose-600 bg-rose-50 dark:bg-rose-900/20'}`}>{event.lab_type}</div>
														<div className="text-[10px] font-bold text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3"/> {event._date.toLocaleDateString()}</div>
													</div>
													<div className="font-bold text-slate-900 dark:text-white text-lg leading-tight mb-3">{event.test_name}</div>
													<div className="text-sm font-medium text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800">{event.result_notes || 'Pending results...'}</div>
												</div>
											</div>
										);
										...
										if (event._type === 'operation') return (
										  <div key={`o-${event.id}`} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
											  <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-white dark:border-slate-900 bg-red-100 text-red-600 shrink-0 md:order-1 md:group-odd:-ml-6 md:group-even:-mr-6 z-10 shadow-sm transition-transform group-hover:scale-110">
											  ...
											  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white dark:bg-slate-950 p-6 rounded-2xl border border-red-200 dark:border-red-900/50 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all">
												  <div className="flex justify-between items-center mb-3">
													<div className="text-[10px] font-black uppercase tracking-widest text-red-600 bg-red-50 dark:bg-red-900/20 px-3 py-1 rounded-full">Surgical Op</div>
													<div className="text-[10px] font-bold text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3"/> {event._date.toLocaleDateString()}</div>
												  </div>
												  ...
												  <div className="flex items-center gap-2 text-xs font-bold text-slate-500 mb-4 bg-slate-50 dark:bg-slate-900 inline-flex px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-800">
													Status: <span className="uppercase text-red-600 flex items-center gap-1"><Activity className="w-3 h-3"/> {event.status.replace('_', ' ')}</span>
												  </div>
												  {event.notes && (
													<div className="text-sm font-medium text-slate-700 dark:text-slate-300 bg-red-50/50 dark:bg-red-900/10 p-3 rounded-xl border border-red-100 dark:border-red-900/30">
													  {event.notes}
													</div>
												  )}
											  </div>
