	...
	try {
	  ...
	  if (tickets) {
		tickets.forEach(ticket => {
		  ...
		  const rate = EXCHANGE_RATES[ticket.currency || 'SAR'] || 1;
	  ...
	  if (transData) {
		transData.forEach(transaction => {
		  ...
		  const rate = EXCHANGE_RATES[transaction.currency || 'SAR'] || 1; 
		  const convertedAmt = amt / rate;

		  if (transaction.type === 'income') manualRev += convertedAmt;
		  else if (transaction.type === 'expense') opExp += convertedAmt;
		});
	  }
	...
	<div className="relative max-w-7xl mx-auto space-y-8 p-4 md:p-8 font-sans min-h-screen overflow-hidden">
	  ...
	  <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-6 border-b border-slate-200 dark:border-slate-800/60">
		<div>
		  <div className="flex items-center gap-2 mb-1">
			...
			<span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('Live Financial Ledger')}</span>
		  </div>
		  <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">{t('Corporate Treasury')}</h1>
		</div>
	  ...
	  {loading ? (
		<div className="flex flex-col items-center justify-center h-64 gap-4">
		  ...
		  <p className="text-sm font-bold text-slate-500 animate-pulse uppercase tracking-widest">{t('Compiling Records...')}</p>
		</div>
	  ) : (
		<div className="space-y-8 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
		  ...
		  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
			<div className="bg-white/80 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm backdrop-blur-xl hover:-translate-y-1 transition-transform">
			  <div className="flex items-center gap-2 mb-4"><div className="bg-emerald-50 dark:bg-emerald-900/30 p-2 rounded-lg"><TrendingUp className="w-4 h-4 text-emerald-600 dark:text-emerald-400"/></div><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('Gross Revenue')}</div></div>
			  <div className="text-2xl lg:text-3xl font-black text-slate-900 dark:text-white truncate">{formatCurrency(metrics.revenue)}</div>
			</div>

			<div className="bg-white/80 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm backdrop-blur-xl hover:-translate-y-1 transition-transform">
			  <div className="flex items-center gap-2 mb-4"><div className="bg-amber-50 dark:bg-amber-900/30 p-2 rounded-lg"><Wallet className="w-4 h-4 text-amber-600 dark:text-amber-400"/></div><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('Gross Payroll')}</div></div>
			...
			<div className="bg-white/80 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm backdrop-blur-xl hover:-translate-y-1 transition-transform">
			  <div className="flex items-center gap-2 mb-4"><div className="bg-red-50 dark:bg-red-900/30 p-2 rounded-lg"><TrendingDown className="w-4 h-4 text-red-600 dark:text-red-400"/></div><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('Operating Exp')}</div></div>
			...
			<div className={`p-6 rounded-3xl shadow-lg backdrop-blur-xl border hover:-translate-y-1 transition-transform ${metrics.netProfit >= 0 ? 'bg-slate-900 dark:bg-black border-emerald-500/30' : 'bg-red-950 border-red-500/30'}`}>
			  <div className="flex items-center gap-2 mb-4"><div className="bg-white/10 p-2 rounded-lg"><DollarSign className="w-4 h-4 text-white"/></div><div className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{t('Net Income (P&L)')}</div></div>
		  ...
		  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
			...
			<div className="lg:col-span-1 space-y-6 md:space-y-8">
			  <div className="bg-white/80 dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm backdrop-blur-xl p-6">
				<h2 className="font-bold text-slate-800 dark:text-white flex items-center gap-2 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800"><PlusCircle className="w-5 h-5 text-emerald-500"/> {t('Log Transaction')}</h2>
				<form onSubmit={handleRegisterTransaction} className="space-y-5">
				  <div className="grid grid-cols-2 gap-3">
					<div>
					  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">{t('Type')}</label>
					  <select value={transForm.type} onChange={e=>setTransForm({...transForm, type: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3 rounded-xl text-sm font-bold dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/50">
						<option value="expense">{t('Liability / Exp')}</option><option value="income">{t('Revenue')}</option>
					  </select>
					</div>
					<div>
					  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">{t('Currency')}</label>
				  ...
				  <div>
					<label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">{t('Amount')}</label>
				  ...
				  <div>
					<label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">{t('Category & Memo')}</label>
					<div className="space-y-2">
						<input required type="text" value={transForm.category} onChange={e=>setTransForm({...transForm, category: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder={t('Category (e.g. IT, Legal)')}/>
						<input required type="text" value={transForm.description} onChange={e=>setTransForm({...transForm, description: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/50" placeholder={t('Memo Description')}/>
					</div>
				  </div>
				  <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black py-4 rounded-xl transition-all active:scale-95 shadow-lg shadow-emerald-500/20">{t('POST TO LEDGER')}</button>
				</form>
			  </div>
			</div>
			...
			<div className="lg:col-span-2 space-y-6 md:space-y-8">
			  <div className="bg-white/80 dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm backdrop-blur-xl overflow-hidden flex flex-col h-[600px]">
				<div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20">
				  <h2 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><ArrowRightLeft className="w-5 h-5 text-slate-500"/> {t('Master Ledger')}</h2>
				  <span className="text-xs font-bold text-slate-500 bg-white dark:bg-slate-900 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700">{transactions.length} {t('Records')}</span>
				</div>
				<div className="overflow-y-auto flex-1 p-4 space-y-3 custom-scrollbar">
				  {transactions.map(transaction => {
					// Convert historical transactions to display currency
					const originalRate = EXCHANGE_RATES[transaction.currency || 'SAR'] || 1;
					const baseUsd = parseFloat(transaction.amount) / originalRate;
					const displayAmt = baseUsd * EXCHANGE_RATES[displayCurrency];

					return (
					<div key={transaction.id} className="flex justify-between items-center p-4 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl hover:shadow-md transition-all group">
					  <div className="flex items-center gap-4">
						<div className={`p-3 rounded-xl ${transaction.type === 'income' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10' : 'bg-red-50 text-red-600 dark:bg-red-500/10'}`}>
						  {transaction.type === 'income' ? <TrendingUp className="w-5 h-5"/> : <TrendingDown className="w-5 h-5"/>}
						</div>
						<div>
						  <div className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 transition-colors">{transaction.description}</div>
						  <div className="flex gap-2 items-center mt-1.5">
							<span className="text-[9px] font-black text-slate-500 uppercase tracking-wider bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">{transaction.category}</span>
							<span className="text-[10px] text-slate-400 font-mono flex items-center gap-1"><CreditCard className="w-3 h-3"/> {t('Original:')} {transaction.currency || 'SAR'} {parseFloat(transaction.amount).toLocaleString()}</span>
						  </div>
						</div>
					  </div>
					  <div className="text-right">
						<div className={`font-mono font-black text-lg ${transaction.type === 'income' ? 'text-emerald-600' : 'text-red-600'}`}>
							{transaction.type === 'income' ? '+' : '-'}{formatCurrency(displayAmt)}
						</div>
						<div className="text-[10px] text-slate-400 font-mono mt-1">{new Date(transaction.transaction_date).toLocaleDateString()}</div>
					  </div>
					</div>
				  )})}
				  {transactions.length === 0 && (
					  <div className="text-center py-20 text-slate-400 font-bold text-sm">{t('No transactions logged.')}</div>
				  )}
				</div>
			  </div>
			</div>
