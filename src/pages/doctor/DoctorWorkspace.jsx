...
export default function DoctorDashboard() {
  ...
  const loadTicketData = async (ticketId) => {
	...
	try {
	  ...
	  if (ticketError || !ticket) throw new Error(t("Invalid MRN or Ticket not found."));

	  const { data: patient, error: patientError } = await supabase.from('patient_files').select('*').eq('id', ticket.patient_id).single();
	  if (patientError) throw new Error(t("Could not retrieve patient master file."));
  ...
  const handlePushToPharmacy = async (e) => {
	...
	try {
	  ...
	  showMessage('success', `${t('Consultation logged. Ticket #')}${activeTicket.id} ${t('routed to Pharmacy.')}`);
	  setActiveTicket(null); setPatientProfile(null); setSearchMrn('');
	  setConsult({ diagnosis: '', symptoms: '', prescribed_meds: '' });
	  fetchDoctorQueue();
	} catch (err) { showMessage('error', t("Error saving consultation: ") + err.message); } finally { setLoading(false); }
  };
	  ...
	  <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-6 border-b border-slate-200 dark:border-slate-800/60">
		<div>
		  <div className="flex items-center gap-2 mb-1">
			...
			<span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('Physician Operations')}</span>
		  </div>
		  <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">{t('Clinical Consultation')}</h1>
		</div>
		...
		<form onSubmit={handleSearchMRN} className="flex bg-white dark:bg-slate-900 backdrop-blur-md p-2 rounded-2xl w-full md:w-[400px] border border-slate-300 dark:border-slate-700 shadow-lg">
		  <input type="text" value={searchMrn} onChange={e => setSearchMrn(e.target.value)} className="flex-1 bg-transparent border-none outline-none px-4 text-sm font-bold text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-0" placeholder={t('Scan override / Input MRN...')} />
		  <button type="submit" disabled={loading} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-2"><Search className="w-4 h-4"/> {t('Fetch')}</button>
		</form>
	  </div>
		...
		{!activeTicket && !loading ? (
		  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
			<div className="flex items-center justify-between">
			  <h3 className="font-black text-lg text-slate-800 dark:text-slate-200 flex items-center gap-2"><Clock className="w-5 h-5 text-indigo-500"/> {t('Active Consultation Waitlist')}</h3>
			  <button onClick={fetchDoctorQueue} className="text-xs font-bold text-indigo-600 flex items-center gap-1.5 hover:underline"><RefreshCcw className="w-3.5 h-3.5"/> {t('Re-sync Live Triage')}</button>
			</div>
			...
			{pendingQueue.length === 0 ? (
			  <div className="h-[400px] border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl flex flex-col items-center justify-center text-slate-400 bg-white/40 dark:bg-slate-900/40">
				...
				<p className="font-bold uppercase tracking-widest text-sm">{t('All Clinical Queues Clear')}</p>
			  </div>
			) : (
				  ...
				  <div key={ticket.id} onClick={() => loadTicketData(ticket.id)} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group">
					<div className="flex justify-between items-start mb-4">
					  ...
					  <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${ticket.status === 'triage_pending' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700'}`}>
						{ticket.status === 'triage_pending' ? t('Bypass Nurse') : t('Ready')}
					  </span>
					</div>
					<h4 className="font-black text-lg text-slate-900 dark:text-white mb-1">{ticket.patient_name || t('Unknown Patient')}</h4>
					<p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 mb-4 line-clamp-1">{ticket.services_requested}</p>
					<div className="text-[10px] font-mono text-slate-400">{t('MRN ID:')} {String(ticket.id)}</div>
				  </div>
				))}
			  </div>
			)}
		  ...
		  <div className="h-[500px] flex items-center justify-center text-indigo-500 font-bold uppercase tracking-widest animate-pulse">{t('Retrieving Encrypted File...')}</div>
		  ...
		  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
			  ...
			  <div className="bg-white/80 dark:bg-slate-900/80 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 backdrop-blur-xl">
				...
				<div className="space-y-4 text-sm mb-6">
				  <div className="grid grid-cols-2 gap-4">
					<div><div className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{t('DOB')}</div><div className="font-bold">{patientProfile.date_of_birth}</div></div>
					<div><div className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{t('Sex / Blood')}</div><div className="font-bold">{patientProfile.sex} | {patientProfile.blood_group}</div></div>
				  </div>
				  <div><div className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">{t('Medical History')}</div><div className="text-slate-600 dark:text-slate-400 italic text-xs">{patientProfile.medical_history || t('None')}</div></div>
				</div>
				...
				<div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl p-4 border border-indigo-100 dark:border-indigo-800/50">
				  <div className="flex justify-between items-center mb-3">
					<h4 className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-1.5"><Activity className="w-3 h-3"/> {t('Triage Vitals')}</h4>
					{activeTicket.status === 'triage_pending' && <span className="text-[9px] font-bold bg-amber-100 text-amber-700 px-2 py-0.5 rounded uppercase">{t('Nurse Bypassed')}</span>}
				  </div>
				  <div className="grid grid-cols-2 gap-3 font-mono text-sm">
					<div><span className="text-[9px] text-slate-500 block uppercase sans-serif">{t('BP')}</span> <span className="font-bold">{vitals.blood_pressure || '--'}</span></div>
					<div><span className="text-[9px] text-slate-500 block uppercase sans-serif">{t('HR')}</span> <span className="font-bold">{vitals.heart_rate || '--'}</span></div>
					<div><span className="text-[9px] text-slate-500 block uppercase sans-serif">{t('Temp')}</span> <span className="font-bold">{vitals.temperature ? `${vitals.temperature}°C` : '--'}</span></div>
					<div><span className="text-[9px] text-slate-500 block uppercase sans-serif">{t('Weight')}</span> <span className="font-bold">{vitals.weight_kg ? `${vitals.weight_kg}kg` : '--'}</span></div>
				  </div>
				  <div className="mt-3 text-xs text-indigo-800 dark:text-indigo-200 border-t border-indigo-200/50 dark:border-indigo-800 pt-2"><strong>{t('Nurse Notes:')}</strong> {activeTicket.nurse_notes || t('No notes provided.')}</div>
				</div>
				<button onClick={() => { setActiveTicket(null); fetchDoctorQueue(); }} className="mt-6 w-full text-xs font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest text-center">{t('← Release File & Return to Waitlist')}</button>
			  </div>
			</div>
			...
			<div className="lg:col-span-2">
			  <div className="bg-white/80 dark:bg-slate-900/80 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 backdrop-blur-xl h-full">
				<h2 className="text-xl font-black text-slate-900 dark:text-white mb-6 flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-4"><ClipboardList className="w-5 h-5 text-indigo-500"/> {t('Examination & Diagnosis')}</h2>
				...
				<form onSubmit={handlePushToPharmacy} className="space-y-6">
				  ...
				  <div>
					<div className="flex justify-between items-end mb-3">
					  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Reported Symptoms')}</label>
					  <button type="button" onClick={() => toggleVoiceDictation('symptoms')} className={`px-4 py-2 rounded-xl border-2 text-sm font-black flex items-center gap-2 transition-all shadow-sm active:scale-95 ${isListening === 'symptoms' ? 'border-red-500 text-red-600 bg-red-50 dark:bg-red-500/10 animate-pulse' : 'border-indigo-200 dark:border-indigo-800 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'}`}>
						{isListening === 'symptoms' ? <><MicOff className="w-5 h-5"/> {t('Recording...')}</> : <><Mic className="w-5 h-5"/> {t('Dictate')}</>}
					  </button>
					</div>
					<input required autoComplete="off" list="symptoms-dict" type="text" value={consult.symptoms} onChange={e => setConsult({...consult, symptoms: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500/50 text-slate-900 dark:text-white" placeholder={t('Type symptoms or use dropdown suggestions...')} />
				  </div>
				  
				  <div>
					<div className="flex justify-between items-end mb-3">
					  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1"><BookOpen className="w-3 h-3"/> {t('Physician Diagnosis')}</label>
					  <button type="button" onClick={() => toggleVoiceDictation('diagnosis')} className={`px-4 py-2 rounded-xl border-2 text-sm font-black flex items-center gap-2 transition-all shadow-sm active:scale-95 ${isListening === 'diagnosis' ? 'border-red-500 text-red-600 bg-red-50 dark:bg-red-500/10 animate-pulse' : 'border-indigo-200 dark:border-indigo-800 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'}`}>
						{isListening === 'diagnosis' ? <><MicOff className="w-5 h-5"/> {t('Recording...')}</> : <><Mic className="w-5 h-5"/> {t('Dictate')}</>}
					  </button>
					</div>
					<input required autoComplete="off" list="diagnosis-dict" type="text" value={consult.diagnosis} onChange={e => setConsult({...consult, diagnosis: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-sm font-black outline-none focus:ring-2 focus:ring-indigo-500/50 text-slate-900 dark:text-white" placeholder={t('Search standard ICD codes...')} />
				  </div>
				  
				  <div>
					<div className="flex justify-between items-end mb-3">
					  <label className="block text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">{t('Prescribed Medications (Rx)')}</label>
					  <button type="button" onClick={() => toggleVoiceDictation('prescribed_meds')} className={`px-4 py-2 rounded-xl border-2 text-sm font-black flex items-center gap-2 transition-all shadow-sm active:scale-95 ${isListening === 'prescribed_meds' ? 'border-red-500 text-red-600 bg-red-50 dark:bg-red-500/10 animate-pulse' : 'border-indigo-200 dark:border-indigo-800 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'}`}>
						{isListening === 'prescribed_meds' ? <><MicOff className="w-5 h-5"/> {t('Recording...')}</> : <><Mic className="w-5 h-5"/> {t('Dictate')}</>}
					  </button>
					</div>
					<input required autoComplete="off" list="rx-dict" type="text" value={consult.prescribed_meds} onChange={e => setConsult({...consult, prescribed_meds: e.target.value})} className="w-full bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-200 dark:border-indigo-800/50 rounded-xl p-4 text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-500/50 text-indigo-900 dark:text-indigo-100 placeholder:text-indigo-400" placeholder={t('Search operational formulary...')} />
				  </div>
				  ...
				  <button type="submit" disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-xl shadow-lg shadow-indigo-500/30 transition-all active:scale-[0.98] flex justify-center items-center gap-2">
					{loading ? t('Processing...') : <>{t('Sign off & Route to Pharmacy')} <ChevronRight className="w-5 h-5"/></>}
				  </button>
				</form>
			  </div>
			</div>
